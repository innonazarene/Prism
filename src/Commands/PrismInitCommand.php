...
class PrismInitCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($tables as $table) {
            ...
            $this->line("     ✓ Models/{$className}.php");

            if ($services) {
                $svcGen->generate($className);
                $this->line("     ✓ Services/{$className}Service.php");
            }
            ...
            if ($policies) {
                ...
                $this->line("     ✓ Policies/{$className}Policy.php");
            }
        }
    ...
    private function checkAndInstallPackages(): void
    {
        ...
        if (! File::exists($lockPath)) {
            $this->warn('composer.lock not found — skipping package check.');
            return;
        }
